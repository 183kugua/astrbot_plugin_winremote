...
if errorlevel 1 (
    echo  [INFO] 当前状态: 服务未安装
) else (
    for /f "tokens=*" %%a in ('nssm status WinRemoteAgent 2^>nul') do echo  [INFO] 当前状态: %%a
)
...
if errorlevel 1 (
    echo [WARN] 启动失败，可能服务未安装。请先选 [5] 安装。
) else (
    echo [OK] 启动命令已发送
...
:stop
echo.
echo [STOP] 正在停止 WinRemoteAgent...
nssm stop WinRemoteAgent 2>nul
if errorlevel 1 (
    echo [WARN] 停止失败，可能服务未安装或未运行。
) else (
    echo [OK] 停止命令已发送
    timeout /t 2 /nobreak >nul
    nssm status WinRemoteAgent
...
:restart
echo.
echo [RESTART] 正在重启 WinRemoteAgent...
nssm restart WinRemoteAgent 2>nul
if errorlevel 1 (
    echo [WARN] 重启失败，可能服务未安装。请先选 [5] 安装。
) else (
    echo [OK] 重启命令已发送
...
if not exist "%AGENT_DIR%winremote_agent.py" (
    echo [WARN] 未找到 winremote_agent.py
...
nssm status WinRemoteAgent >nul 2>&1
if not errorlevel 1 (
    echo [WARN] 服务已存在，请先卸载（选 [6]）再重新安装
    pause
    goto menu
)
...
if not exist "%RUN_SCRIPT%" (
    echo [WARN] 未找到 run_agent.bat 启动脚本
...
:uninstall
echo.
echo [WARN] 确认卸载 WinRemoteAgent 服务？[y/N]
set "ans="
set /p "ans="
if /i not "!ans!"=="y" (
    echo 已取消
...
if exist "%RUN_SCRIPT%" (
    ...
    notepad "%RUN_SCRIPT%"
    echo.
    echo [WARN] 修改配置后请重启服务（选 [3]）使更改生效
) else (
    echo [WARN] 未找到 run_agent.bat，请先安装服务（选 [5]）
)
echo.
pause
goto menu
