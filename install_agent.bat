...
if "!tok_choice!"=="2" (
    ...
    if "!TOKEN!"=="请换成你的长随机Token" (
        echo   [FAIL] 不能使用占位文本，请输入真实 Token！
        goto manual_token_input
    )
    call :check_len "!TOKEN!" tok_len
    if !tok_len! LSS 16 (
        echo   [WARN] Token 长度不足 16 位（当前 !tok_len! 位），安全性较低！
        echo   建议使用 ≥16 位随机字符串。
        set /p "SHORT_OK=仍然使用？[y/N]: "
        if /i not "!SHORT_OK!"=="y" goto manual_token_input
    )
    echo   [OK] Token 已确认（长度 !tok_len! 位）
)
...
for /f "tokens=2" %%v in ('python --version 2^>^&1') do echo [OK] 已检测到 Python %%v
echo.
